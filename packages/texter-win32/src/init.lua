...
--- The UTF-16 a line is, and the byte of the UTF-8 line each unit of it starts at: what Uniscribe
--- counts a line in is units of its own, and what a caret, a click and a selection are about is the
--- byte of a Lua string. A character outside the basic plane is two units, and both of them come
--- from the same byte of the line.
---@param text string
---@return WCHAR* wide
---@return number[] bytes # The byte each unit starts at, from one
---@return number count
...
function Face:inkOf(glyph, pixelHeight)
	...
	local metrics = ffi.new("GLYPHMETRICS")
	local transform = ffi.new("MAT2", FIXED_ONE, 0, 0, FIXED_ONE)
...
function win32.shape(face, text, pixelHeight, _opts)
	...
	local characters, bytes, count = wideOf(text)
	...
	local items = ffi.new("SCRIPT_ITEM[?]", count + 1)
	...
	local function byteOf(unit)
		local at = bytes[unit + 1]

		return (at or (#text + 1)) - 1
	end

	local glyphs, runs, x = {}, {}, 0.0
	...
	for index = 0, used[0] - 1 do
		...
		local cache = ffi.new("SCRIPT_CACHE[1]")

		local out = ffi.new("WORD[?]", length * 3 + 16)
		local clusters = ffi.new("WORD[?]", length)
		local attributes = ffi.new("SCRIPT_VISATTR[?]", length)
		local shaped = ffi.new("int[1]")

		local result = usp.ScriptShape(face.context, cache, characters + first, length, length * 3 + 16, analysis,
			out, clusters, attributes, shaped)

		assert(result == 0 or result == E_GLYPH_MISSING, "Uniscribe could not shape a run of " .. text)

		local advances = ffi.new("int[?]", shaped[0])
		local offsets = ffi.new("GOFFSET[?]", shaped[0])
		...
		if rtl and logical then
			...
			for at = 0, shaped[0] - 1 do
				total = total + advances[at]
			end

			local trailing = total

			for at = shaped[0] - 1, 0, -1 do
				trailing = trailing - advances[at]

				glyphs[#glyphs + 1] = {
					glyph = out[at],
					cluster = byteOf(clusters[at] + first),
					x = x + trailing + offsets[at].du,
					y = -offsets[at].dv,
					advance = advances[at],
				}
			end
